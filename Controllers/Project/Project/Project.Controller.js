...
const GetProjectShortDetailsExtra = async (req, res) => {
  try {
    // 1. Get IDs of library projects already used as templates
    const projectsWithTemplates = await Project.find({ 
      original_project_id: { $exists: true, $ne: null } 
    }).select("original_project_id").lean();
    
    let usedIds = projectsWithTemplates.map(p => p.original_project_id?.toString()).filter(Boolean);
    
    // If we want to ALLOW a specific ID (e.g. during Edit mode), remove it from usedIds
    const allowId = req.query.allowId;
    if (allowId) {
      usedIds = usedIds.filter(id => id !== allowId);
    }

    // 2. Filter: Only show "Library" sites (those WITHOUT the field)
    //    OR the explicitly allowed ID.
    const findProjectDetails = await Project.find({ 
      $or: [
        { original_project_id: { $exists: false } },
        ...(allowId ? [{ _id: allowId }] : [])
      ],
      _id: { $nin: usedIds } 
    }).lean();
    ...
    return ResponseOk(res, 200, "Projects retrieved successfully", findProjectDetails);
  } catch (error) {
    console.error("Error in GetProjectShortDetailsExtra:", error);
    return ErrorHandler(res, 500, "Failed to retrieve project short details", error);
  }
};

const GetProjectDetailsByIdExtra = async (req, res) => {
  try {
    const projectId = req.query.projectId;
    if (!projectId) {
    ...
    const elevator = await Elevators.find({ project_id: projectId }).lean();
    const findProjectDetails = await Project.findById(projectId).lean();

    return ResponseOk(res, 200, "Project details retrieved successfully", {
      projectDetails: findProjectDetails,
      elevatorDetails: elevator,
    });
  } catch (error) {
    console.error("Error in GetProjectDetailsByIdExtra:", error);
    return ErrorHandler(res, 500, "Failed to retrieve project details", error);
  }
};
...
module.exports = {
  ...
  DeleteProject,
  GetProjectShortDetailsExtra,
  GetProjectDetailsByIdExtra,
};
